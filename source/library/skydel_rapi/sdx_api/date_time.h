...
{
  DateTime(int pYear, int pMonth, int pDay, int pHour = 0, int pMinute = 0, int pSecond = 0) :
  ...
  DateTime(int pHour = 0, int pMinute = 0, int pSecond = 0) : Date(), hour(pHour), minute(pMinute), second(pSecond) {}
...
struct parse_json<Sdx::DateTime>
{
  ...
  {
    return value.IsObject() && value.HasMember("Year") && parse_json<int>::is_valid(value["Year"]) &&
           value.HasMember("Month") && parse_json<int>::is_valid(value["Month"]) && value.HasMember("Day") &&
           parse_json<int>::is_valid(value["Day"]) && value.HasMember("Hour") &&
           parse_json<int>::is_valid(value["Hour"]) && value.HasMember("Minute") &&
           parse_json<int>::is_valid(value["Minute"]) && value.HasMember("Second") &&
           parse_json<int>::is_valid(value["Second"]);
  }
