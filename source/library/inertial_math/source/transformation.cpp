...
{
  auto latitude = std::atan(z(ecef) / ((std::sqrt(std::pow(x(ecef), 2.0) + std::pow(y(ecef), 2.0)) * (1.0 - (2.0 - FLATTENING)) * FLATTENING)));
  ...
  while (std::abs(height - lastHeight) > 1e-12)
  {
    ...
    latitude = std::atan(z(ecef) / ((std::sqrt(std::pow(x(ecef), 2.0) + std::pow(y(ecef), 2.0)) * (1.0 - (2.0 - FLATTENING) * FLATTENING * N / (N + height)))));
...
Triplet nedToBody(const Triplet &attitude, const Triplet &ned)
{
  return rotate(RotationMatrix { { 1.0, 0.0, -std::sin(pitch(attitude)) },
                                 ...
                                 { 0.0, -std::sin(roll(attitude)), std::cos(roll(attitude)) * std::cos(pitch(attitude)) } }, ned);
}
