...
const QJsonArray ANGULAR_VELOCITY = { "Angular Velocity Roll (rad/s)", "Angular Velocity Pitch (rad/s)", "Angular Velocity Yaw (rad/s)" };

const QString ImuLogger::csvHeader = ELAPSED_TIME + ',' +
                                     ACCELERATION.at(0).toString() + ',' +
                                     ACCELERATION.at(1).toString() + ',' +
                                     ACCELERATION.at(2).toString() + ',' +
                                     ANGULAR_VELOCITY.at(0).toString() + ',' +
                                     ANGULAR_VELOCITY.at(1).toString() + ',' +
...
const QString ImuLogger::jsonHeader = QJsonDocument(QJsonObject({ { ELASPED_JSON_KEY, ELAPSED_TIME},
                                                                  ...
                                                                  { ANGULAR_VELOCITY_JSON_KEY, ANGULAR_VELOCITY } } )).toJson();

ImuLogger::ImuLogger(ImuDataFormat dataFormat) :
...
QString ImuLogger::getHeader() const
{
  switch (m_dataFormat)
  {
    case ImuDataFormat::CSV: return csvHeader;
    case ImuDataFormat::JSON: return jsonHeader;
...
QString ImuLogger::getFormattedData(const Iml::InertialData& data) const
{
  switch (m_dataFormat)
  {
    case ImuDataFormat::CSV: return getFormattedDataInCsvFormat(data);
    case ImuDataFormat::JSON: return getFormattedDataInJsonFormat(data);
     default: return "INVALID";
  }
}
